...
-(BOOL) AppendOpenZipFile:(NSString*) zipFile {
...
-(NSString*)fileNameFromChar:(const char*)fileName {
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	do{
		...
		NSString * strPath = [self fileNameFromChar:filename];
...
// Utility method
+(BOOL)extract:(NSString *)zipFile toDir:(NSString *)path {
...
+(BOOL)archiveInner:(ZipArchive*)za dir:(NSString*)dir dirName:(NSString*)dirName {
    NSFileManager *fm = [NSFileManager defaultManager];
    NSArray *files = [fm contentsOfDirectoryAtPath:dir error:NULL];
    for (NSString* file in files) {
        NSString* path = [dir stringByAppendingPathComponent:file];
        NSString* name = [dirName stringByAppendingPathComponent:file];
        BOOL isdir;
        [fm fileExistsAtPath:path isDirectory:&isdir];
        if (isdir) {
            if (![self archiveInner:za dir:path dirName:name]) {
                return NO;
            }
        } else {
            if (![za addFileToZip:path newname:name]) {
                return NO;
            }
        }
    }
...
// Utility method
// dir/*をzipアーカイブする
+(BOOL)archive:(NSString *)dir toZip:(NSString *)path {
    ZipArchive* za = [[ZipArchive alloc]init];
    BOOL success = NO;
    if ([za CreateZipFile2:path]) {
        success = [self archiveInner:za dir:dir dirName:@""];
        [za CloseZipFile2];
    }
    [za release];
    return success;
